...
class CircularLinkedList:
    """
    CircularLinkedList is a data structure where the last element points back to the first.
    ...
    def __repr__(self) -> str:
        ...
        Return the string representation of the list.
        """
        return f"CircularLinkedList({self.__str__()})"
    ...
    def append(self, value) -> None:
        ...
        if not self.head:
            ...
            new_node.next = self.head  # Connect back to head
    ...
    def remove(self, index) -> None:
        ...
        if index == 0:
            ...
            # Remove tail (find previous node)
            current = self.head
            while current.next != self.tail:
                current = current.next
    ...
    def rotate(self, steps=1) -> None:
        ...
        if steps > 0:
            # Rotate right - head moves forward
            for _ in range(steps):
                self.head = self.head.next
                self.tail = self.tail.next
        else:
            # Rotate left - head moves backward
            steps = abs(steps)
            for _ in range(steps):
                self.head = self.tail
                # Find new tail
...
class TestCircularLinkedList(unittest.TestCase):
    ...
    def test_append(self):
        ...
        self.assertEqual(self.list.tail.next, self.list.head)  # Circular reference
        self.assertEqual(len(self.list), 1)
        
        self.list.append(2)
        self.assertEqual(self.list.head.value, 1)
        self.assertEqual(self.list.tail.value, 2)
        self.assertEqual(self.list.tail.next, self.list.head)  # Tail points to head
    ...
    def test_remove(self):
        ...
        self.list.remove(0)
        self.assertEqual(len(self.list), 1)
        self.assertEqual(self.list.traverse(), [3])
        self.assertEqual(self.list.tail.next, self.list.head)  # Still circular
    ...
    def test_circular_property(self):
        # Test that you can traverse the entire list by following next pointers
        for i in range(1, 6):
            self.list.append(i)
